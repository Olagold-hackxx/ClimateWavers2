...
model_h5 = tf.keras.models.load_model("computer_vision_model/model/disaster_detector.h5")
...
image = cv2.imread(filename="inference_test/test_computer_vision_model/images/image2.jpeg")
...
# For classification tasks with softmax activation, you might want to get class probabilities and labels
predicted_class_probabilities = tf.nn.softmax(predictions, axis=1)
predicted_class = tf.argmax(predicted_class_probabilities, axis=1).numpy()[0]
...
result_classes.sort()
print(f"The H5 model depicts {result_classes[predicted_class]}")
